...
data = web.DataReader("DTB3", "fred", start, end).resample("M").first()
data["CPI"] = web.DataReader("CPIAUCNS", "fred", start, end).resample("M").first()
data = data.rename(columns={"DTB3":"3 Month Treasury"})
data["Inflation (CPI)"] = (data["CPI"].diff(12) / data["CPI"]) * 100
print(data)
