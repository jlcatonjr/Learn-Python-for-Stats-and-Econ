...
class RPG():    
    ...
    def __init__(self, num_humans, num_computers):
    ...
    def create_players(self,):
        for h in range(self.num_humans):
            self.agent_dict[h] = agent1.Agent( rpg = self, hp = 20, mp = 10, strength = 5, defense = 5, magic = 5, id_num = h, computer = False)
            self.human_ids.append(h)
        lowest_computer_id = h + 1
        for c in range(lowest_computer_id, lowest_computer_id +  self.num_computers):
            self.agent_dict[c] = agent1.Agent(rpg = self, hp = 20, mp = 10, strength = 5, defense = 5, magic = 5, id_num = c, computer = True)
