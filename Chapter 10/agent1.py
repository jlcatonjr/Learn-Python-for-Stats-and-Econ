...
class Agent():
    ...
    def __init__(self, rpg, hp, mp, strength, defense, magic, id_num, computer = True):
        self.rpg = rpg
        self.id_num = id_num
        self.alive = True
        ...
        if computer == False:
            ...
            self.type = "human"
        if computer:
            self.strength = random.randint(int(.5 * strength), int(strength))
            self.hp = random.randint(int(.5 * hp), int(hp))
            self.max_hp = hp
            self.mp = random.randint(int(.5 * mp), int(mp))
            self.max_mp = mp
            self.defense = random.randint(int(.5 * defense), int(defense))
            self.magic = random.randint(int(.5 * magic), int(magic))
            self.type = "computer"
